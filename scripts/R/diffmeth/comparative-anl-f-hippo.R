...
# covered in all samples.
 ...
 meth.f = unite(filtered.myobj.f, destrand = FALSE)

# take a look at the meth object

    head(meth.f)

# create object meth.f as dataframe to try to bind to others meth.n to do a proper correlation graph    
...
# 
#     jpeg("correlation.f.jpg")
#     getCorrelation(meth.f, plot = TRUE)
#     dev.off()

## Clustering Samples
    
# get a cluster of the samples based in the methylation similarities
# method of this cluster is ward.f(see hclust() function), using pearson as correlation distance

#### WARNING!!!!!!####
# here i will have to investigate how to do it using all 4 samples

    # jpeg("cluster.f.jpg")
    # clusterSamples(meth.f, dist = "correlation", method = "ward", plot = TRUE) 
    # dev.off()
...
# so I'm going to apply this variable construction:
    ...
    # hc.f = clusterSamples(meth.f, dist="correlation", method="ward", plot=FALSE)

# this way I am storing this information in the variable hc.f
...
# Scree plot: A scree plot graphs the amount of variation explained by each component
    ...
    # jpeg("screeplot.f.jpg")
    # PCASamples(meth.f, screeplot=TRUE)
    # dev.off()

# Scatter plot of PCA analysis

    # jpeg("scatterplot.pca.f.jpg")
    # PCASamples(meth.f)
    # dev.off()
...
# or not of a treatment difference between the samples.
  ...
  myDiff.f = calculateDiffMeth(meth.f, overdispersion="MN")

# the myDiff.f variable will contain all the q-values for the selected hypothesis testing

# I would take a look at myDiff.f data with
# head(myDiff.f)

# Save myDiff.f as a dataframe in the folder
   
# out <- capture.output(as.fata.frame(myDiff.f))
#  
# cat(out, file="diff-meth-all.f.fsv", sep=",", append=TRUE, fill = 5)
...
myDiff25p.hyper.f = getMethylDiff(myDiff.f, difference = 25, qvalue = 0.01, type = "hyper", save.db = F)

# out <- capture.output(as.data.frame(myDiff25p.hyper.f))
# options(max.print = .Machine$integer.max) 
# cat(out, file="diff-meth-hyper-25p-q001.f.txt", sep=" ", append=TRUE, fill = 5)
    
# get hypomethylated bases

# difference of 25%

myDiff25p.hypo.f = getMethylDiff(myDiff.f, difference = 25, qvalue = 0.01, type = "hypo", save.db = F)

# out <- capture.output(as.data.frame(myDiff25p.hypo.d))
# options(max.print = .Machine$integer.max)
# cat(out, file="diff-meth-hypo-25p-q001.f.txt", sep=" ", append=TRUE, fill = 5)

# difference of 50%

# myDiff50p.hypo.f = getMethylDiff(myDiff.f, difference = 50, qvalue = 0.01, type = "hypo", save.db = T)

# out <- capture.output(as.data.frame(myDiff50p.hypo.f))
# options(max.print = .Machine$integer.max)
# cat(out, file="diff-meth-hypo-50p-q001.f.txt", sep=" ", append=TRUE, fill = 5)
...
myDiff25p.f = getMethylDiff(myDiff.f, difference = 25, qvalue = 0.01, save.db = F)

# out <- capture.output(as.data.frame(myDiff25p.f))
# options(max.print = .Machine$integer.max)
# cat(out, file="diff-meth-25p-q001.f.txt", sep=" ", append=TRUE, fill = 5)

# 50%

# myDiff50p.f = getMethylDiff(myDiff.f, difference = 50, qvalue = 0.01)

# out <- capture.output(as.fata.frame(myDiff50p.f))
# options(max.print = .Machine$integer.max)
# cat(out, file="diff-meth-50p-q001.f.txt", sep=" ", append=TRUE, fill = 5)


# we can check the distribution of hypo/hypermethylated bases per chromosome:

#    outPerChr.f <- capture.output(as.data.frame(
#    diffMethPerChr(myDiff.f ,plot=FALSE,qvalue.dutoff = 0.01, meth.dutoff = 25)
#    ))
#    options(max.print = .Machine$integer.max)
#    cat(outPerChr.d, file="output-diff-meth-per-chr.d.txt", sep=" ", fill = 5)

# get the plot too

    jpeg("diff-meth-per-chr-ovd.f.jpg")
    diffMethPerChr(myDiff.f, plot = TRUE, qvalue.cutoff = 0.01, meth.cutoff = 25)
    dev.off()
