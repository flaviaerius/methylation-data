...
# covered in all samples.
 ...
 meth.e = unite(filtered.myobj.e, destrand = FALSE)

# take a look at the meth object

    head(meth.e)

# create object meth.e as dataframe to try to bind to others meth.n to do a proper correlation graph    
...
# 
#     jpeg("correlation.e.jpg")
#     getCorrelation(meth.e, plot = TRUE)
#     dev.off()

## Clustering Samples
    
# get a cluster of the samples based in the methylation similarities
# method of this cluster is ward.e(see hclust() function), using pearson as correlation distance

#### WARNING!!!!!!####
# here i will have to investigate how to do it using all 4 samples

    # jpeg("cluster.e.jpg")
    # clusterSamples(meth.e, dist = "correlation", method = "ward", plot = TRUE) 
    # dev.off()
...
# so I'm going to apply this variable construction:
    ...
    # hc.e = clusterSamples(meth.e, dist="correlation", method="ward", plot=FALSE)

# this way I am storing this information in the variable hc.e
...
# Scree plot: A scree plot graphs the amount of variation explained by each component
    ...
    # jpeg("screeplot.e.jpg")
    # PCASamples(meth.e, screeplot=TRUE)
    # dev.off()

# Scatter plot of PCA analysis

    # jpeg("scatterplot.pca.e.jpg")
    # PCASamples(meth.e)
    # dev.off()
...
# or not of a treatment difference between the samples.
  ...
  myDiff.e = calculateDiffMeth(meth.e, overdispersion="MN")

# the myDiff.e variable will contain all the q-values for the selected hypothesis testing

# I would take a look at myDiff.e data with
# head(myDiff.e)

# Save myDiff.e as a dataframe in the folder
   
# out <- capture.output(as.eata.frame(myDiff.e))
#  
# cat(out, file="diff-meth-all.e.esv", sep=",", append=TRUE, fill = 5)
...
myDiff25p.hyper.e = getMethylDiff(myDiff.e, difference = 25, qvalue = 0.01, type = "hyper", save.db = F)

# get hypomethylated bases

# difference of 25%

myDiff25p.hypo.e = getMethylDiff(myDiff.e, difference = 25, qvalue = 0.01, type = "hypo", save.db = F)

# get all differentially methylated bases

# 25%

myDiff25p.e = getMethylDiff(myDiff.e, difference = 25, qvalue = 0.01, save.db = F)

# out <- capture.output(as.data.frame(myDiff25p.e))
# options(max.print = .Machine$integer.max)
# cat(out, file="diff-meth-25p-q001.e.txt", sep=" ", append=TRUE, fill = 5)

# 50%

# myDiff50p.e = getMethylDiff(myDiff.e, difference = 50, qvalue = 0.01)

# out <- capture.output(as.eata.frame(myDiff50p.e))
# options(max.print = .Machine$integer.max)
# cat(out, file="diff-meth-50p-q001.e.txt", sep=" ", append=TRUE, fill = 5)


# we can check the distribution of hypo/hypermethylated bases per chromosome:

#    outPerChr.e <- capture.output(as.data.frame(
#    diffMethPerChr(myDiff.e ,plot=FALSE,qvalue.dutoff = 0.01, meth.dutoff = 25)
#    ))
#    options(max.print = .Machine$integer.max)
...
# get the plot too
     ...
     jpeg("diff-meth-per-chr-ovd.e.jpg")
     diffMethPerChr(myDiff.e, plot = TRUE, qvalue.cutoff = 0.01, meth.cutoff = 25)
     dev.off()
