...
# covered in all samples.
 ...
 meth.d = unite(filtered.myobj.d, destrand = FALSE)

# take a look at the meth object

    head(meth.d)

# create object meth.d as dataframe to try to bind to others meth.n to do a proper correlation graph    
...
# 
#     jpeg("correlation.d.jpg")
#     getCorrelation(meth.d, plot = TRUE)
#     dev.off()
...
# here i will have to investigate how to do it using all 4 samples
    ...
    # jpeg("cluster.d.jpg")
    # clusterSamples(meth.d, dist = "correlation", method = "ward", plot = TRUE) 
    # dev.off()
...
# so I'm going to apply this variable construction:
    ...
    # hc.d = clusterSamples(meth.d, dist="correlation", method="ward", plot=FALSE)

# this way I am storing this information in the variable hc.d
...
# Scree plot: A scree plot graphs the amount of variation explained by each component
    ...
    # jpeg("screeplot.d.jpg")
    # PCASamples(meth.d, screeplot=TRUE)
    # dev.off()

# Scatter plot of PCA analysis

    # jpeg("scatterplot.pca.d.jpg")
    # PCASamples(meth.d)
    # dev.off()
...
# or not of a treatment difference between the samples.
  ...
  myDiff.d = calculateDiffMeth(meth.d, overdispersion="MN")

# the myDiff.d variable will contain all the q-values for the selected hypothesis testing

# I would take a look at myDiff.d data with
# head(myDiff.d)

# Save myDiff.d as a dataframe in the folder
   
# out <- capture.output(as.data.frame(myDiff.d))
#  
# cat(out, file="diff-meth-all.d.dsv", sep=",", append=TRUE, fill = 5)
...
myDiff25p.hyper.d = getMethylDiff(myDiff.d, difference = 25, qvalue = 0.01, type = "hyper", save.db = F)

# out <- capture.output(as.data.frame(myDiff25p.hyper.d))
# options(max.print = .Machine$integer.max) 
# cat(out, file="diff-meth-hyper-25p-q001.d.txt", sep=" ", append=TRUE, fill = 5)
    
# get hypomethylated bases

# difference of 25%

myDiff25p.hypo.d = getMethylDiff(myDiff.d, difference = 25, qvalue = 0.01, type = "hypo", save.db = F)

# out <- capture.output(as.data.frame(myDiff25p.hypo.d))
# options(max.print = .Machine$integer.max)
# cat(out, file="diff-meth-hypo-25p-q001.d.txt", sep=" ", append=TRUE, fill = 5)

# difference of 50%

# myDiff50p.hypo.d = getMethylDiff(myDiff.d, difference = 50, qvalue = 0.01, type = "hypo", save.db = T)

# out <- capture.output(as.data.frame(myDiff50p.hypo.d))
# options(max.print = .Machine$integer.max)
# cat(out, file="diff-meth-hypo-50p-q001.d.txt", sep=" ", append=TRUE, fill = 5)
...
myDiff25p.d = getMethylDiff(myDiff.d, difference = 25, qvalue = 0.01, save.db = F)

# out <- capture.output(as.data.frame(myDiff25p.d))
# options(max.print = .Machine$integer.max)
# cat(out, file="diff-meth-25p-q001.d.txt", sep=" ", append=TRUE, fill = 5)

# 50%

# myDiff50p.d = getMethylDiff(myDiff.d, difference = 50, qvalue = 0.01)

# out <- capture.output(as.data.frame(myDiff50p.d))
# options(max.print = .Machine$integer.max)
# cat(out, file="diff-meth-50p-q001.d.txt", sep=" ", append=TRUE, fill = 5)


# we can check the distribution of hypo/hypermethylated bases per chromosome:

#    outPerChr.d <- capture.output(as.data.frame(
#    diffMethPerChr(myDiff.d ,plot=FALSE,qvalue.dutoff = 0.01, meth.dutoff = 25)
#    ))
#    options(max.print = .Machine$integer.max)
...
# get the plot too
     ...
     jpeg("diff-meth-per-chr-ovd.d.jpg")
     diffMethPerChr(myDiff.d, plot = TRUE, qvalue.cutoff = 0.01, meth.cutoff = 25)
     dev.off()
