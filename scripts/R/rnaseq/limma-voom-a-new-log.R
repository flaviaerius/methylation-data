...
info_a.edger <- factor(c( rep("Ma", 2) , rep("4C", 3)))
info_a.edger <- relevel(info_a.edger , ref = "Ma")
edgeR.DGElist <- DGEList(counts = df_a , group = info_a.edger)
...
design <- model.matrix(~info_a.edger)
...
DGE.results_limma <- topTable(voomed.fitted, coef = "info_a.edger4C",
...
write.table(DGE.results_limma_anno, "DGE.results_limma_anno_a.txt", quote = F,
              row.names = F, sep = "\t")
...
write.table(DGE.results_notNA, "DGE.results_limma_anno_notNA_a.txt", 
            quote = F,
            row.names = F, sep = "\t")

# save subset only with gene symbols and logFC (for GSEA preranked)
preranked <- subset(DGE.results_notNA, select= c("SYMBOL", "logFC"))
write.table(preranked, "preranked_geneexpr_a.txt", quote = F,
            row.names = F, sep = "\t")


save.image("limma-voom-a.RData")
