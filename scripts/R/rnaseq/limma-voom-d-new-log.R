...
info_d.edger <- factor(c(rep("Ma", 2) , rep("4C11+", 3)))
info_d.edger <- relevel(info_d.edger , ref = "Ma")
edgeR.DGElist <- DGEList(counts = df_d , group = info_d.edger)
...
design <- model.matrix(~info_d.edger)
...
DGE.results_limma <- topTable(voomed.fitted, coef = "info_d.edger4C11+",
...
write.table(DGE.results_limma_anno, "DGE.results_limma_anno_d.txt", quote = F,
              row.names = F, sep = "\t")
...
write.table(DGE.results_notNA, "DGE.results_limma_anno_notNA_d.txt", 
            quote = F,
            row.names = F, sep = "\t")

# save subset only with gene symbols and logFC (for GSEA preranked)
preranked <- subset(DGE.results_notNA, select= c("SYMBOL", "logFC"))
write.table(preranked, "preranked_geneexpr_d.txt", quote = F,
            row.names = F, sep = "\t")


save.image("limma-voom-d.RData")
