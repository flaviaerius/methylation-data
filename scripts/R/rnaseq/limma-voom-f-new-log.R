...
info_f.edger <- factor(c(rep("Ma", 2) , rep("4C11-", 3)))
info_f.edger <- relevel(info_f.edger , ref = "Ma")
edgeR.DGElist <- DGEList(counts = df_f , group = info_f.edger)
...
design <- model.matrix(~info_f.edger)
...
DGE.results_limma <- topTable(voomed.fitted, coef = "info_f.edger4C11-",
...
write.table(DGE.results_limma_anno, "DGE.results_limma_anno_f.txt", quote = F,
              row.names = F, sep = "\t")
...
write.table(DGE.results_notNA, "DGE.results_limma_anno_notNA_f.txt", 
            quote = F,
            row.names = F, sep = "\t")

# save subset only with gene symbols and logFC (for GSEA preranked)
preranked <- subset(DGE.results_notNA, select= c("SYMBOL", "logFC"))
write.table(preranked, "preranked_geneexpr_f.txt", quote = F,
            row.names = F, sep = "\t")


save.image("limma-voom-f.RData")
