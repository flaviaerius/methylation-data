...
info_b.edger <- factor(c( rep("4C", 3) , rep("4C11-", 3)))
info_b.edger <- relevel(info_b.edger , ref = "4C")
edgeR.DGElist <- DGEList(counts = df_b , group = info_b.edger)
...
design <- model.matrix(~info_b.edger)
...
DGE.results_limma <- topTable(voomed.fitted, coef = "info_b.edger4C11-",
...
write.table(DGE.results_limma_anno, "DGE.results_limma_anno_b.txt", quote = F,
              row.names = F, sep = "\t")
...
write.table(DGE.results_notNA, "DGE.results_limma_anno_notNA_b.txt", 
            quote = F,
            row.names = F, sep = "\t")

# save subset only with gene symbols and logFC (for GSEA preranked)
preranked <- subset(DGE.results_notNA, select= c("SYMBOL", "logFC"))
write.table(preranked, "preranked_geneexpr_b.txt", quote = F,
            row.names = F, sep = "\t")


save.image("limma-voom-b.RData")
