...
info_c.edger <- factor(c(rep("4C11-", 3) , rep("4C11+", 3)))
info_c.edger <- relevel(info_c.edger , ref = "4C11-")
edgeR.DGElist <- DGEList(counts = df_c , group = info_c.edger)
...
design <- model.matrix(~info_c.edger)
...
DGE.results_limma <- topTable(voomed.fitted, coef = "info_c.edger4C11+",
...
write.table(DGE.results_limma_anno, "DGE.results_limma_anno_c.txt", quote = F,
              row.names = F, sep = "\t")
...
write.table(DGE.results_notNA, "DGE.results_limma_anno_notNA_c.txt", 
            quote = F,
            row.names = F, sep = "\t")

# save subset only with gene symbols and logFC (for GSEA preranked)
preranked <- subset(DGE.results_notNA, select= c("SYMBOL", "logFC"))
write.table(preranked, "preranked_geneexpr_c.txt", quote = F,
            row.names = F, sep = "\t")


save.image("limma-voom-c.RData")
